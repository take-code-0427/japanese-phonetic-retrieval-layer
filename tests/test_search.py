...
def test_default_search_keeps_the_ann_path(sample_searcher: PhoneticSearcher) -> None:
    ...
    経路の取り違えを捕まえるために、上位の並びを固定する。2 位が「仕組」でなく
    「仕組み」なのは代表選びの同点解消による (`_representative_rank` 参照)。
    サンプル語彙はこの 2 つに同じ一般性を与えてあるので (`conftest` 参照)、
    表層の符号順で決まる。以前はここに到着順の偶然で「仕組」が入っていた。

    **3 位以降は固定しない。** サンプル語彙は 18 語しかないので、3 位には音韻的に
    遠い語 (「科学」音韻 0.559) が一般性だけで入ってくる。実辞書では母集団が
    十分あって起きないことなので、ここで縛ると重みを触るたびに実態と無関係な
    修正を強いられる。
    """
    assert surfaces(sample_searcher, "乳首", limit=2) == ["手首", "仕組み"]
