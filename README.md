...
検索        1 クエリ 100〜260ms (ANN 候補 5000 件 + rerank)
```

候補数を 400 に絞れば 16ms まで落ちるが、202 万語ではコサイン 0.91 以上の語が
400 件を超えるため、「乳首」に対する「手首」のような明らかな近傍が候補から
漏れる。既定は品質を優先して 5000 件にしている。
...
乳首 -> チクビ [ch i k u b i] 3 モーラ  (181ms, preset=pun)
...
- **編集距離の高速化**: rerank では 1 クエリあたり数千件に編集距離を掛けるため、
  ...
  さらに編集距離を除いた成分でスコアの上限を先に求め、上位が確定した時点の
  最低スコアを打ち切り線に使って見込みのない候補には距離を計算しない。
